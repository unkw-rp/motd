...
switch($orderby)
{
	...
	case 7: $order = 'Infections DESC, Nick ASC'; $underline8 = 'style=text-decoration:underline;'; $desc8 = '▾'; break;
	case 8: $order = 'Damage DESC, Nick ASC'; $underline9 = 'style=text-decoration:underline;'; $desc9 = '▾'; break;
	case 9: $order = 'Ammo DESC, Nick ASC'; $underline10 = 'style=text-decoration:underline;'; $desc10 = '▾'; break;
	case 10: $order = '`Zombie Wins` DESC, Nick ASC'; $underline11 = 'style=text-decoration:underline;'; $desc12 = '▾'; break;
	case 11: $order = '`Human Wins` DESC, Nick ASC'; $underline12 = 'style=text-decoration:underline;'; $desc11 = '▾'; break;
	case 12: $order = 'Level DESC, XP DESC'; $underline13 = 'style=text-decoration:underline;'; $desc13 = '▾'; break;
	case 13: $order = '(Kills - Deaths) DESC, Assists DESC, Headshots DESC, `Human Wins` DESC, `Zombie Wins` DESC, Infections DESC, Ammo DESC, Damage DESC, XP DESC, Nick ASC'; break;
	...
	case 21: $order = 'Infections ASC, Nick ASC'; $underline8 = 'style=text-decoration:underline;'; $desc8 = '▴'; break;
	case 22: $order = 'Damage ASC, Nick ASC'; $underline9 = 'style=text-decoration:underline;'; $desc9 = '▴'; break;
	case 23: $order = 'Ammo ASC, Nick ASC'; $underline10 = 'style=text-decoration:underline;'; $desc10 = '▴'; break;
	case 24: $order = '`Zombie Wins` ASC, Nick ASC'; $underline11 = 'style=text-decoration:underline;'; $desc12 = '▴'; break;
	case 25: $order = '`Human Wins` ASC, Nick ASC'; $underline12 = 'style=text-decoration:underline;'; $desc11 = '▴'; break;
...
<table>
    <tr id="a">
        ...
        <th><a id=url href="'.$main_url.'top15.php?top='.$top.'&player='.$player.'&style='.$style.'&order='.$num12.'&default_order='.$default_order.'&page='.$page.'&db_table1='.$db_table1.'&db_table2='.$db_table2.'&search='.htmlspecialchars($search).'" '.$underline12.'><img border="0" src='.$main_url.'css/img/icon-trophy.png id=himg2></img>'.$desc11.'</a></th>
        <th><a id=url href="'.$main_url.'top15.php?top='.$top.'&player='.$player.'&style='.$style.'&order='.$num11.'&default_order='.$default_order.'&page='.$page.'&db_table1='.$db_table1.'&db_table2='.$db_table2.'&search='.htmlspecialchars($search).'" '.$underline11.'><img border="0" src='.$main_url.'css/img/icon-star.png id=himg2></img>'.$desc12.'</a></th>
        ...
        <th id="v"><a id=url href="'.$main_url.'top15.php?top='.$top.'&player='.$player.'&style='.$style.'&order='.$num13.'&default_order='.$default_order.'&page='.$page.'&db_table1='.$db_table1.'&db_table2='.$db_table2.'&search='.htmlspecialchars($search).'" '.$underline13.'>Rank'.$desc13.'</a></th>
   			...
   			if(!empty($search)) {
   				$placeholder = '%'.$search.'%';
   				$top15 = DB::run('SELECT Player, Nick, `Steam ID`, IP, XP, Level, Kills, Assists, Headshots, Deaths, Infections, Ammo, Damage, `Human Wins`, `Zombie Wins`, Skill, `Skill Range`, Flags, Online, New, Steam, Avatar FROM '.$db_table1.' WHERE Nick LIKE ? OR IP LIKE ? OR `Steam ID` LIKE ? ORDER BY '.$order.' LIMIT '.$items_per_page.' OFFSET '.$offset.'', [$placeholder, $placeholder, $placeholder])->FetchAll(PDO::FETCH_ASSOC);
   			} else {
    			$top15 = DB::run('SELECT Player, Nick, `Steam ID`, IP, XP, Level, Kills, Assists, Headshots, Deaths, Infections, Ammo, Damage, `Human Wins`, `Zombie Wins`, Skill, `Skill Range`, Flags, Online, New, Steam, Avatar FROM '.$db_table1.' ORDER BY '.$order.' LIMIT '.$items_per_page.' OFFSET '.$offset.'')->FetchAll(PDO::FETCH_ASSOC);
    		}
    		$i = $offset + 1;
    		$table = false;
    		...
    		foreach ($top15 as $row) {
				...
				echo '
						...
						<td>'.$row['Human Wins'].'</td>
						<td>'.$row['Zombie Wins'].'</td>
						<td>'.$row['Damage'].'</td>
