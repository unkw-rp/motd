...
if(isset($mapstats['Team Win'])) {
	switch($mapstats['Team Win']) {
	    case 0:
	        $team = 5;
	        $team_name = 'Zombies Win';
	        break;
	    case 1:
	        $team = 6;
	        $team_name = 'Humans Win';
	        break;
	    case 2:
	        $team = 7;
	        $team_name = 'Match Draw';
...
$sql_mapstats = DB::run('SELECT `Player`, Nick, `Steam ID`, IP, `Map Kills`, `Map Deaths`, `Map Infections`, `Map XP`, Level, Flags, Online, New, Steam, Avatar FROM '.htmlspecialchars($_GET['db_table3']).' ORDER BY `Map Kills` DESC, `Map Deaths` ASC, `Map Infections` DESC LIMIT 5')->FetchAll(PDO::FETCH_ASSOC);
$i = 1;
$table = false;
	...
	echo '
		'.$tr.'
		    ...
		    <td>
	            ...
	            <td id=a><a>'.$mapstats['Map Infections'].'</a></td>
	            <td id="r'.$level.'"></td>
		    </td>
		</tr>
	';
